...
$2"; shift 2 ;;
        ...
        # 2..42 is the header comment block: it ENDS at the bash-3.2 note, the line before
...
if [ "$SKIP_BUILD" -eq 1 ] || [ "$DRY_RUN" -eq 1 ]; then
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
        ...
        # `sopack pack` refuses without --allow-helper-log.
...
SUMMARY="$OUTDIR/summary.md"
{
    ...
    printf '> packed with `--allow-helper-log`. They log each target name, `.text` address and\n'
