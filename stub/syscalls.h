/*
 * syscalls.h - freestanding Linux syscall wrappers for arm64, x86_64, arm
 * (EABI).
 *
 * The stub is injected into arbitrary .so files, so it must not depend on libc
 * being reachable through a PLT/GOT that we did not set up. We therefore issue
 * raw syscalls via inline assembly and implement the few libc-ish helpers we
 * need (memcpy) ourselves. No external symbols => no dynamic relocations in the
 * blob.
 */
...
#if defined(__aarch64__)
static inline long sopk_syscall6(long n, long a, long b, long c, long d, long e,
                                 long f) {
...
#define sopk_syscall2(n, a, b)                                                 \
  sopk_syscall6((n), (long)(a), (long)(b), 0, 0, 0, 0)
#define sopk_syscall3(n, a, b, c)                                              \
  sopk_syscall6((n), (long)(a), (long)(b), (long)(c), 0, 0, 0)
#define sopk_syscall4(n, a, b, c, d)                                           \
  sopk_syscall6((n), (long)(a), (long)(b), (long)(c), (long)(d), 0, 0)
#define sopk_syscall5(n, a, b, c, d, e)                                        \
  sopk_syscall6((n), (long)(a), (long)(b), (long)(c), (long)(d), (long)(e), 0)
...
/* Raw syscalls return -errno on failure (a value in [-4095, -1]); detect that
 * range rather than only == MAP_FAILED (-1), or an EINVAL/EPERM would slip
 * through. */
...
/* mmap anonymous RW at a FIXED address (fallback for when mremap onto the .text
 * VA is rejected: unmap the file-backed .text, then map fresh anon pages at the
 * same VA). */
#define SOPK_MAP_FIXED 0x10
static inline void *sopk_mmap_fixed_anon(void *addr, size_t len) {
#if defined(__arm__)
  return (void *)sopk_syscall6(
      SOPK_NR_mmap2, (long)addr, (long)len, SOPK_PROT_READ | SOPK_PROT_WRITE,
      SOPK_MAP_PRIVATE | SOPK_MAP_ANONYMOUS | SOPK_MAP_FIXED, -1, 0);
#else
  return (void *)sopk_syscall6(
      SOPK_NR_mmap, (long)addr, (long)len, SOPK_PROT_READ | SOPK_PROT_WRITE,
      SOPK_MAP_PRIVATE | SOPK_MAP_ANONYMOUS | SOPK_MAP_FIXED, -1, 0);
#endif
}
...
static inline size_t sopk_page_size(void) {
  long fd = sopk_syscall4(SOPK_NR_openat, -100 /*AT_FDCWD*/, "/proc/self/auxv",
                          SOPK_O_RDONLY, 0);
...
static inline void sopk_clear_icache(void *start, void *end) {
   ...
   * __builtin___clear_cache, which can lower to a libcall to __clear_cache and
   * would break the "no external symbols" property of the injected blob. Line
   * sizes come from CTR_EL0 (DminLine / IminLine, in log2 words).
   */
...
#else
  (void)start;
  (void)end; /* x86_64: coherent I-cache, nothing to do */
#endif
}
